...
enum combos { // Combos to make a new one you need to change the COMBO COUNT in Config.h
  ...
  ZOOMO, // zoomout
  ZOOM0, // zoom to 100
  ZENMO, // activate zen mode
  DUPLI, // duplicate combo
...
const uint16_t PROGMEM palpri_combo[] = {KC_Q, KC_P, COMBO_END}; // Command Palette & browser
const uint16_t PROGMEM zenmode_combo[] = {KC_Z, KC_M, COMBO_END}; // Activate zenmode
const uint16_t PROGMEM duplic_combo[] = {KC_J, KC_D, COMBO_END}; // Duplicate selected word
...
combo_t key_combos[COMBO_COUNT] = { // Combo Functions - this is what the combo's listed above will do when struck.
  ...
  [ZENMO] = COMBO(zenmode_combo, ZENMODE), // Zen Mode
  [PALPC] = COMBO(palpri_combo, PRVWIN), // Private Browser window and Command Palette
  [DUPLI] = COMBO(duplic_combo, DUPLCTE), // ctrl+d
  // brush increase
  // brush decrease
};
